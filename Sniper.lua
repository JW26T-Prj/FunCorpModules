...
function eventLoop(p,f)
	...
	if f <= 5000 and f >= 4400 and vivo >= 2 then
		eventWin()
		showMessage("<N>Tempo esgotado! <VP><b>"..vivos.."</b> <N>ratos permaneceram vivos.",nil)
	end
	if f <= 500 then
		tfm.exec.newGame(mapas[math.random(#mapas)])
	end
...
function eventSummoningStart(name)
	showMessage("<R>Não é permitido o uso de objetos de shaman! Use somente a seta pra baixo.",name)
end
function eventSummoningEnd(name)
	tfm.exec.killPlayer(name)
end
function eventNewGame()
	showMessage("<R>Não é permitido o uso de objetos de shaman! Use somente a seta pra baixo.",nil)
	vivo=0; prox=false; obterJogadores(); enabled=true; remain=10;
	for name,player in next,tfm.get.room.playerList do
		if name:sub(1,1) == "*" then
			tfm.exec.killPlayer(name)
			showMessage("<R>Você precisa estar logado no Transformice para poder participar deste jogo.",name)
...
function eventNewPlayer(name)
	...
	showMessage("<VP><b>Sniper!</b>\n<N>Sobreviva às bigornas do shaman! Criado por Patrick_mahomes#1795.\n\nversão 1.9: limpeza de código",name)
end
for name,player in next,tfm.get.room.playerList do
	eventNewPlayer(name)
end
